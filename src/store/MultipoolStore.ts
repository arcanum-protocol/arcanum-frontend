...
class MultipoolStore {
    ...
    async withdrawFees(to: Address) {
        const { request } = await this.multipool.simulate.withdrawFees([to], { account: this.userAddress });

        await this.walletClient.writeContract(request);
    }

    async togglePause() {
        const { request } = await this.multipool.simulate.togglePause({ account: this.userAddress });
    ...
    async setSharePriceTTL(newSharePriceTTL: number) {
        const { request } = await this.multipool.simulate.setSharePriceTTL([BigInt(newSharePriceTTL)], { account: this.userAddress });

        await this.walletClient.writeContract(request);
    }

    async toggleForcePushAuthority(authority: Address) {
        const { request } = await this.multipool.simulate.toggleForcePushAuthority([authority], { account: this.userAddress });

        await this.walletClient.writeContract(request);
    }

    async toggleTargetShareAuthority(authority: Address) {
        const { request } = await this.multipool.simulate.toggleTargetShareAuthority([authority], { account: this.userAddress });

        await this.walletClient.writeContract(request);
    }

    async increaseCashback(address: Address) {
        const { request } = await this.multipool.simulate.increaseCashback([address], { account: this.userAddress });
